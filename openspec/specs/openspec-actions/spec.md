# openspec-actions Specification

## Purpose

Running OpenSpec commands on behalf of the user through a single runner that never waits for input, never reaches outside the open project, and reports a failure with the command, the exit status, and the real output.
